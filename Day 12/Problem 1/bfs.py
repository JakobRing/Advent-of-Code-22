...
if __name__ == "__main__":
    ...
    while queue:
        ...
        # dijkstra
        if curr_y > 0 and d[curr_y-1, curr_x] == -1 and (elevation[curr_y, curr_x] - elevation[curr_y-1, curr_x]) >= -1:
            # has element above that isn't reached already
            d[curr_y - 1, curr_x] = curr_d + 1
            queue.append((curr_y-1, curr_x))
        if curr_y < elevation.shape[0]-1 and d[curr_y+1, curr_x] == -1 and (elevation[curr_y, curr_x] - elevation[curr_y+1, curr_x]) >= -1:
            # has element below that isn't reached already
            d[curr_y + 1, curr_x] = curr_d + 1
            queue.append((curr_y + 1, curr_x))
        if curr_x > 0 and d[curr_y, curr_x-1] == -1 and (elevation[curr_y, curr_x] - elevation[curr_y, curr_x-1]) >= -1:
            # has element above that isn't reached already
            d[curr_y, curr_x - 1] = curr_d + 1
            queue.append((curr_y, curr_x-1))
        if curr_x < elevation.shape[1]-1 and d[curr_y, curr_x+1] == -1 and (elevation[curr_y, curr_x] - elevation[curr_y, curr_x+1]) >= -1:
            # has element below that isn't reached already
            d[curr_y, curr_x + 1] = curr_d + 1
            queue.append((curr_y, curr_x+1))
    print(d)
