...
class Dir:
    ...
    def get_smallest_size_with_at_least(self, minsize: int) -> int:
        if self.get_size() >= minsize:
            smallest = self.get_size()
            for d in self.subdirs:
                if d.get_smallest_size_with_at_least(minsize):
                    d_size = d.get_smallest_size_with_at_least(minsize)
                    smallest = min(smallest, d_size)
        else:
            smallest = None
        return smallest
