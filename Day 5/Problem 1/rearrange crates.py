    ...
    while line:             # line = "move {count} from {starting_point} to {destination+1}"
        ...
        if command[0] != 'move' or command[2] != 'from' or command[4] != 'to':
            ...
            for _ in range(int(command[1])):
                # last element of string to the end of the destination string + remove at starting_point
                crate_stacks[int(command[5])-1] += crate_stacks[int(command[3])-1][-1]
                crate_stacks[int(command[3])-1] = crate_stacks[int(command[3])-1][:-1]
        line = next(f, None)
    print(crate_stacks)
